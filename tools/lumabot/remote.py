...
REMOTE_HELP = (
    ...
    "/lumabot agent\n"
    "/lumabot off\n\n"
    "Forward and backward continue until STOP or another control. "
...
def execute_remote_action(
    ...
    *,
    ...
    if action == "drive" and continuous:
        data = SCHEDULER.start_continuous(direction, throttle)
    else:
        data = SCHEDULER.start(direction, throttle, duration)
    if data.get("error"):
        return {"ok": False, "text": data["error"], "data": data}
    if action == "drive" and continuous:
        label = f"{direction.capitalize()} latched. Press STOP or choose another control."
    else:
